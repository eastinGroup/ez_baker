...
for num_id, x in enumerate([x for x in __globals if x.startswith('map_')]):
    for y in [item for item in dir(__globals[x]) if item.startswith('EZB_Map_') and item != 'EZB_Map_Handplane']:
...
EZB_Maps_Handplane = type("EZB_maps_Handplane", (bpy.types.PropertyGroup,), {'maps':maps ,'__annotations__': map_annotations})

classes = maps + [EZB_Maps_Handplane]

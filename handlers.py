...
def unregister():
    try:
        old_func = next(x for x in bpy.app.handlers.depsgraph_update_post if x.__name__ == update_group_objects.__name__)
        bpy.app.handlers.depsgraph_update_post.remove(old_func)
